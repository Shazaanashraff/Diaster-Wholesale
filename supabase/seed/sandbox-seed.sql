...
--
-- Run by scripts/sandbox-reset.mjs AFTER sandbox.reset_all(), with
-- `search_path = sandbox` already set for the session — table names
-- below are intentionally unqualified so they resolve against the
-- sandbox schema and never accidentally touch `public`.
--
-- Idempotent: every insert uses fixed UUIDs + ON CONFLICT, so running
-- this twice in a row (without a reset in between) never duplicates rows.
-- ================================================================

-- ────────────────────────────────────────────────────────────────
-- LOCATIONS (reset_all() truncates these too, so they must be reseeded)
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- CUSTOMERS (Walk-in first — required by every POS sale as a fallback)
-- ────────────────────────────────────────────────────────────────
...
-- Invoice 1: Nimal Electronics — wholesale, paid (the "confirmed" invoice)
...
-- EXPENSES
-- ────────────────────────────────────────────────────────────────
INSERT INTO expenses (id, category, description, amount, method, reference, created_at) VALUES
  ('11000000-0000-0000-0000-000000000001', 'rent',      'Warehouse monthly rent — April',     85000.00, 'bank_transfer', 'RENT-APR-2026', now() - interval '30 days'),
  ('11000000-0000-0000-0000-000000000002', 'utilities', 'Electricity bill — CEB March',       12400.00, 'cash',          'CEB-MAR-2026',  now() - interval '25 days'),
  ('11000000-0000-0000-0000-000000000003', 'transport', 'Delivery van fuel — week 1',          8500.00, 'cash',          '',              now() - interval '18 days'),
  ('11000000-0000-0000-0000-000000000004', 'salaries',  'Staff salaries — April',            125000.00, 'bank_transfer', 'SAL-APR-2026',  now() - interval '15 days'),
  ('11000000-0000-0000-0000-000000000005', 'utilities', 'Internet & phone bill',               4200.00, 'cash',          'SLT-APR-2026',  now() - interval '10 days'),
  ('11000000-0000-0000-0000-000000000006', 'transport', 'Delivery charges — Matara run',       6800.00, 'cash',          '',              now() - interval '5 days'),
  ('11000000-0000-0000-0000-000000000007', 'supplies',  'Packaging material & stationery',     3500.00, 'cash',          '',              now() - interval '3 days'),
  ('11000000-0000-0000-0000-000000000008', 'rent',      'Warehouse monthly rent — May',       85000.00, 'bank_transfer', 'RENT-MAY-2026', now() - interval '1 day')
ON CONFLICT (id) DO NOTHING;

-- ────────────────────────────────────────────────────────────────
-- SUPPLIER PAYMENTS
-- ────────────────────────────────────────────────────────────────
INSERT INTO supplier_payments (id, supplier_id, purchase_id, amount, method, notes, paid_at) VALUES
  ('21000000-0000-0000-0000-000000000001', 'a1000000-0000-0000-0000-000000000001', 'd1000000-0000-0000-0000-000000000001',
   500000.00, 'bank_transfer', 'Advance payment for PO0001', now() - interval '25 days'),
  ('21000000-0000-0000-0000-000000000002', 'a1000000-0000-0000-0000-000000000001', 'd1000000-0000-0000-0000-000000000001',
   540250.00, 'bank_transfer', 'Final settlement PO0001',   now() - interval '18 days')
ON CONFLICT (id) DO NOTHING;

-- ================================================================
-- DONE — sandbox now has:
--   2 locations, 3 suppliers, 12 products, 7 customers (incl. Walk-in),
--   2 purchases, 12 stock batches, 5 invoices (1 paid = "confirmed"),
