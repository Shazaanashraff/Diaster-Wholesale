...
-- Run by scripts/sandbox-reset.mjs after sandbox.reset_all().
-- Idempotent: every insert uses ON CONFLICT DO NOTHING; every id is a
-- fixed UUID so E2E tests and dropdown fixtures are deterministic.
--
-- Curated from the root sandbox-seed.sql, with one fix: the app hardcodes
-- a "Walk-in" customer at id c1000000-0000-0000-0000-000000000001
-- (see WALK_IN_CUSTOMER_ID in src/pages/POSPage.tsx) — the original file
-- reused that exact id for a different customer. This version reserves
-- ...001 for Walk-in and shifts the rest down so sandbox POS behaves the
-- same as production for walk-in sales.
-- ================================================================
...
-- ────────────────────────────────────────────────────────────────
-- CUSTOMERS (…001 reserved for Walk-in — matches WALK_IN_CUSTOMER_ID)
-- ────────────────────────────────────────────────────────────────
INSERT INTO sandbox.customers (id, name, phone, email, address, type, credit_limit, outstanding_balance, is_active) VALUES
  ('c1000000-0000-0000-0000-000000000001', 'Walk-in Customer',         '-',          '-',                          '-',                             'retail',         0.00,      0.00, true),
  ...
  ('c1000000-0000-0000-0000-000000000004', 'Sunil Tech Solutions',      '0712345678', 'sunil.tech@gmail.com',     '78 Kandy Rd, Kurunegala',       'wholesale', 200000.00,      0.00, true),
  ('c1000000-0000-0000-0000-000000000005', 'Kamani Silva',              '0754321987', 'kamani.silva@gmail.com',   '23 Temple Rd, Nugegoda',        'retail',      5000.00,      0.00, true),
  ('c1000000-0000-0000-0000-000000000006', 'Ruwan Jayawardena',         '0763456789', 'ruwan.j@hotmail.com',      '5 Lake Rd, Moratuwa',           'retail',      5000.00,   1200.00, true),
  ('c1000000-0000-0000-0000-000000000007', 'City Electronics Matara',   '0412234567', 'cityelectronics@yahoo.com','88 Hakmana Rd, Matara',         'wholesale', 150000.00,  45000.00, true)
ON CONFLICT DO NOTHING;
...
INSERT INTO sandbox.purchases (id, reference, supplier_id, location_id, status, exchange_rate, total_rmb, total_lkr, cost_finalized, notes) VALUES
  ('d1000000-0000-0000-0000-000000000001', 'PO0001',
    ...
    (SELECT id FROM sandbox.locations WHERE name = 'Main Warehouse'),
...
-- ────────────────────────────────────────────────────────────────
INSERT INTO sandbox.stock_batches (id, product_id, cartons, loose_pieces, cost_per_piece, notes, received_at) VALUES
  ...
  ('e1000000-0000-0000-0000-000000000006', 'b1000000-0000-0000-0000-000000000006', 1, 5,    350.00, 'Initial stock',        now() - interval '35 days'),
  ('e1000000-0000-0000-0000-000000000007', 'b1000000-0000-0000-0000-000000000007', 3, 8,    180.00, 'Received from PO0001', now() - interval '20 days'),
  ('e1000000-0000-0000-0000-000000000008', 'b1000000-0000-0000-0000-000000000008', 1, 3,    750.00, 'Received from PO0001', now() - interval '20 days'),
  ('e1000000-0000-0000-0000-000000000009', 'b1000000-0000-0000-0000-000000000009', 0, 3,   5500.00, 'Initial stock',        now() - interval '35 days'),
  ('e1000000-0000-0000-0000-000000000010', 'b1000000-0000-0000-0000-000000000010', 1, 2,   1800.00, 'Initial stock',        now() - interval '35 days'),
  ('e1000000-0000-0000-0000-000000000011', 'b1000000-0000-0000-0000-000000000011', 1, 5,    650.00, 'Initial stock',        now() - interval '35 days'),
  ('e1000000-0000-0000-0000-000000000012', 'b1000000-0000-0000-0000-000000000012', 1, 0,   2200.00, 'Initial stock',        now() - interval '35 days')
ON CONFLICT DO NOTHING;
...
INSERT INTO sandbox.payments (invoice_id, customer_id, amount, method, reference, paid_at) VALUES
  ('f1000000-0000-0000-0000-000000000002', 'c1000000-0000-0000-0000-000000000003',
...
INSERT INTO sandbox.payments (invoice_id, customer_id, amount, method, reference, paid_at) VALUES
  ('f1000000-0000-0000-0000-000000000003', 'c1000000-0000-0000-0000-000000000005',
...
INSERT INTO sandbox.payments (invoice_id, customer_id, amount, method, reference, paid_at) VALUES
  ('f1000000-0000-0000-0000-000000000005', 'c1000000-0000-0000-0000-000000000006',
...
--   3 suppliers, 12 products, 7 customers (incl. Walk-in), 2 purchases,
--   12 stock batches, 5 invoices, 8 expenses, 2 supplier payments
-- ================================================================
