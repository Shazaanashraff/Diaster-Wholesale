-- ================================================================
-- Diastar Wholesale — Loyalty System + Full Returns Module v2
-- Migration: 20260515000000
-- ================================================================

-- =====================================
-- 1. LOYALTY COLUMNS ON CUSTOMERS
-- =====================================
...
CREATE TABLE IF NOT EXISTS loyalty_transactions (
  ...
  return_id         UUID,  -- FK added after sales_returns is created
...
-- =====================================
-- 3. IDEMPOTENCY KEY ON INVOICES
-- =====================================
ALTER TABLE invoices
  ADD COLUMN IF NOT EXISTS idempotency_key TEXT;

CREATE UNIQUE INDEX IF NOT EXISTS invoices_idempotency_key_idx
...
-- =====================================
-- 6. HELPER: AVAILABLE STOCK PIECES
-- =====================================
CREATE OR REPLACE FUNCTION get_available_stock_pieces(p_product_id UUID)
RETURNS INT AS $$
DECLARE
  v_ppc INT;
  v_total_in INT;
  ...
  v_result INT;
BEGIN
  SELECT GREATEST(pieces_per_carton, 1) INTO v_ppc FROM products WHERE id = p_product_id;

  SELECT
    COALESCE(SUM(b.cartons * v_ppc + b.loose_pieces), 0)
  INTO v_total_in
  FROM stock_batches b
  WHERE b.product_id = p_product_id;

  SELECT
    COALESCE(SUM(ii.cartons * v_ppc + ii.pieces), 0)
  INTO v_total_out
  ...
  SELECT
    COALESCE(SUM(sa.adjustment_pieces), 0)
  INTO v_total_adj
  FROM stock_adjustments sa
  WHERE sa.product_id = p_product_id;

  -- Note: the product_stock view uses batch totals vs invoice_items totals
  -- For a fresh availability check inside a transaction, count directly from stock_batches
...
-- =====================================
-- 7. HELPER: RESTORE STOCK PIECES
-- =====================================
CREATE OR REPLACE FUNCTION restore_stock_pieces(
  p_product_id  UUID,
  p_cartons     INT,
  p_pieces      INT,
...
-- =====================================
-- 8. ATOMIC: CREATE SALES RETURN
-- =====================================
CREATE OR REPLACE FUNCTION create_sales_return_atomic(p_payload JSONB)
RETURNS JSONB AS $$
DECLARE
  v_invoice           RECORD;
  v_inv_item          RECORD;
...
BEGIN
  ...
  -- Validate each returned item belongs to this invoice
  ...
  FOR v_ret_item IN SELECT * FROM jsonb_array_elements(COALESCE(p_payload->'returned_items','[]'::jsonb))
  LOOP
    SELECT GREATEST(pieces_per_carton,1) INTO v_ppc FROM products WHERE id = (v_ret_item->>'product_id')::UUID;
  ...
  IF v_return_type = 'Exchange' THEN
    ...
    LOOP
      SELECT GREATEST(pieces_per_carton,1) INTO v_ppc FROM products WHERE id = (v_repl_item->>'product_id')::UUID;
      v_need  := COALESCE((v_repl_item->>'cartons')::INT,0) * v_ppc
               + COALESCE((v_repl_item->>'pieces')::INT,0);
      v_avail := get_available_stock_pieces((v_repl_item->>'product_id')::UUID);
    ...
    INSERT INTO invoices (invoice_no, customer_id, mode, subtotal, discount, total, payment_status, notes)
    VALUES (
      v_exchange_inv_no,
    ...
    -- Insert replacement items and deduct stock
    FOR v_repl_item IN SELECT * FROM jsonb_array_elements(COALESCE(p_payload->'replacement_items','[]'::jsonb))
    LOOP
      SELECT GREATEST(pieces_per_carton,1) INTO v_ppc FROM products WHERE id = (v_repl_item->>'product_id')::UUID;
    ...
    IF ABS(v_exchange_diff) > 0.01 AND (p_payload->>'settlement_method') IS NOT NULL THEN
      INSERT INTO payments (invoice_id, customer_id, amount, method, reference, paid_at)
      VALUES (
        v_exchange_inv_id,
        v_invoice.customer_id,
        v_exchange_diff,  -- positive = customer pays, negative = refund
  ...
  RETURN jsonb_build_object(
    ...
    'return_number', (SELECT return_number FROM sales_returns WHERE id = v_return_id),
    'status', v_status,
...
-- =====================================
-- 9. COMPLETE A PENDING RETURN
-- =====================================
CREATE OR REPLACE FUNCTION complete_sales_return(
  p_return_id       UUID,
  p_resolution_type TEXT,   -- 'Repaired' | 'Replaced'
  p_completed_by    TEXT
)
RETURNS JSONB AS $$
DECLARE
  ...
  LOOP
    ...
    IF v_units > 0 THEN
      IF p_resolution_type = 'Repaired' THEN
        -- Item repaired → goes back to stock
        PERFORM restore_stock_pieces(v_item.product_id, v_item.return_cartons, v_item.return_pieces,
          'Repaired return ' || v_return.return_number);
      ELSE
        -- Replaced → new item deducted from stock (defective item not restocked)
        PERFORM deduct_stock_fifo(v_item.product_id, v_units);
      END IF;
    END IF;
  END LOOP;
...
-- =====================================
-- 10. ATOMIC: UNDO SALES RETURN
-- =====================================
CREATE OR REPLACE FUNCTION undo_sales_return_atomic(
  p_return_id   UUID,
  p_undo_reason TEXT,
  p_undone_by   TEXT
)
...
BEGIN
  ...
  -- Pending → just cancel, no stock changes
  IF v_return.status = 'Pending' THEN
    UPDATE sales_returns
  ...
  -- ── Undo Completed Exchange ───────────────────────────────────
  IF v_return.return_type = 'Exchange' AND v_return.exchange_invoice_id IS NOT NULL THEN
    DELETE FROM invoices WHERE id = v_return.exchange_invoice_id;

    -- Re-deduct returned items (they were restored during exchange)
    FOR v_item IN SELECT * FROM jsonb_array_elements(COALESCE(v_snapshot->'returned_items','[]'::jsonb))
    LOOP
      SELECT GREATEST(pieces_per_carton,1) INTO v_ppc FROM products WHERE id = (v_item->>'product_id')::UUID;
      v_units := COALESCE((v_item->>'return_cartons')::INT,0) * v_ppc
               + COALESCE((v_item->>'return_pieces')::INT,0);
      IF v_units > 0 THEN
        PERFORM deduct_stock_fifo((v_item->>'product_id')::UUID, v_units);
      END IF;
    END LOOP;

    -- Re-restore replacement items (they were deducted during exchange)
  ...
  -- ── Undo Completed Standard Return (Replaced) ─────────────────
  IF v_return.return_type = 'Return' AND v_return.resolution_type = 'Replaced' THEN
    -- Replacement stock was deducted; restore it
    FOR v_item IN SELECT * FROM jsonb_array_elements(COALESCE(v_snapshot->'returned_items','[]'::jsonb))
    LOOP
      SELECT GREATEST(pieces_per_carton,1) INTO v_ppc FROM products WHERE id = (v_item->>'product_id')::UUID;
      PERFORM restore_stock_pieces(
  ...
  -- ── Undo Completed Standard Return (Repaired) ─────────────────
  IF v_return.return_type = 'Return' AND v_return.resolution_type = 'Repaired' THEN
    -- Stock was restored on completion; deduct it back
    FOR v_item IN SELECT * FROM jsonb_array_elements(COALESCE(v_snapshot->'returned_items','[]'::jsonb))
    LOOP
      SELECT GREATEST(pieces_per_carton,1) INTO v_ppc FROM products WHERE id = (v_item->>'product_id')::UUID;
      v_units := COALESCE((v_item->>'return_cartons')::INT,0) * v_ppc
...
-- 11. GRANTS
-- =====================================
GRANT SELECT, INSERT, UPDATE ON loyalty_transactions TO anon, authenticated;
...
GRANT EXECUTE ON FUNCTION create_sales_return_atomic(JSONB) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION complete_sales_return(UUID,TEXT,TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION undo_sales_return_atomic(UUID,TEXT,TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION restore_stock_pieces(UUID,INT,INT,TEXT) TO anon, authenticated;
GRANT EXECUTE ON FUNCTION get_available_stock_pieces(UUID) TO anon, authenticated;
