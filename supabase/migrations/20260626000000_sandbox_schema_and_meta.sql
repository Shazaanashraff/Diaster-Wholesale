...
--
-- Going-forward convention: future migrations that change `public`
-- apply the identical DDL to `sandbox` in the same file.
--
...
-- Loyalty columns (from 20260515000000)
...
-- Cheque float (from 20260625000000)
ALTER TABLE sandbox.customers
...
-- original_units (from 20260623000000)
...
-- Idempotency key (from 20260515000000)
...
-- Salesperson columns (from 20260517000000, 20260524000000)
ALTER TABLE sandbox.invoices
  ADD COLUMN IF NOT EXISTS salesperson_name TEXT;

-- salesperson_id added after salespeople table is created (below)
...
-- Cheque status (from 20260625000000)
...
-- ────────────────────────────────────────────────────────────────
-- 13. RETURNS (customer returns — legacy)
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 27. SALESPEOPLE  (from 20260517000000)
-- ────────────────────────────────────────────────────────────────
...
-- Now that salespeople exists, add salesperson_id FK to invoices (from 20260524000000)
ALTER TABLE sandbox.invoices
  ADD COLUMN IF NOT EXISTS salesperson_id UUID REFERENCES sandbox.salespeople(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS sandbox_idx_invoices_salesperson_id ON sandbox.invoices(salesperson_id);

-- ────────────────────────────────────────────────────────────────
-- 28. LOYALTY TRANSACTIONS  (from 20260515000000)
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 29. SALES RETURNS  (from 20260515000000)
-- ────────────────────────────────────────────────────────────────
...
-- salesperson_id on sales_returns (from 20260623000000)
...
-- ────────────────────────────────────────────────────────────────
-- 30. SALES RETURN ITEMS  (from 20260515000000)
-- ────────────────────────────────────────────────────────────────
...
-- ================================================================
-- GRANTS on all tables (explicit, idempotent)
-- ================================================================
...
-- Supplier return reference generator
...
-- Stock transfer reference generator
...
-- Purchase receive → stock trigger function (updated version with location_id)
...
-- Supplier return complete trigger function
...
-- Supplier payable trigger function
...
-- Stock transfer complete trigger function
...
-- original_units stamp trigger function (from 20260623000000)
CREATE OR REPLACE FUNCTION sandbox.set_stock_batch_original_units()
RETURNS TRIGGER AS $$
DECLARE v_ppc INT;
BEGIN
  SELECT GREATEST(COALESCE(pieces_per_carton, 1), 1) INTO v_ppc
  FROM sandbox.products WHERE id = NEW.product_id;
  NEW.original_units := COALESCE(NEW.cartons, 0) * v_ppc + COALESCE(NEW.loose_pieces, 0);
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SET search_path = sandbox;

-- Get available stock pieces helper (from sandbox-patch.sql)
...
-- Restore stock pieces helper (from sandbox-patch.sql)
...
-- Atomic sales return creator (from sandbox-patch.sql)
...
-- Complete a sales return (from sandbox-patch.sql)
...
-- Undo a sales return atomically (from sandbox-patch.sql)
...
-- Grant execute on helper functions (from sandbox-patch.sql)
...
-- original_units stamp trigger (from 20260623000000)
...
-- ================================================================
-- APP_META — schema marker in BOTH schemas
-- ================================================================

-- public.app_meta
CREATE TABLE IF NOT EXISTS public.app_meta (
  id            INT PRIMARY KEY DEFAULT 1,
  schema_marker TEXT NOT NULL DEFAULT 'public',
...
CREATE TABLE IF NOT EXISTS sandbox.app_meta (
  id            INT PRIMARY KEY DEFAULT 1,
  schema_marker TEXT NOT NULL DEFAULT 'sandbox',
  app_version   TEXT NOT NULL DEFAULT '0.1.54',
  updated_at    TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- ================================================================
-- RESET FUNCTION — schema-locked, service_role only
-- ================================================================
