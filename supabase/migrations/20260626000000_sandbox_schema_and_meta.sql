...
--
-- Convention going forward: any future migration that changes `public`
-- must apply the identical DDL to `sandbox` in the same file, so the two
-- schemas never drift apart.
--
-- This migration ports the ad-hoc root file `sandbox-setup.sql` (26
-- tables + supporting functions/views/triggers/seed data) into version
-- control, and adds:
--   1. an `app_meta` marker table in BOTH `public` and `sandbox`, so the
--      app can assert which schema it is actually talking to at runtime.
--   2. `sandbox.reset_all()` — a SECURITY DEFINER function, schema-locked
--      to `sandbox`, that truncates every sandbox table except `app_meta`.
--      This is the only data-clearing primitive in the Sandbox feature and
--      it is structurally impossible for it to touch `public`.
--
...
-- ────────────────────────────────────────────────────────────────
-- Re-grant on already-existing objects (ALTER DEFAULT PRIVILEGES above
-- only covers objects created after it ran in this same session/role;
-- this covers tables that already existed from a prior ad-hoc run).
-- ────────────────────────────────────────────────────────────────
...
-- ================================================================
-- SEED DATA (idempotent: guarded with NOT EXISTS instead of the bare
-- ON CONFLICT DO NOTHING from the original script, since `customers`
-- has no unique constraint on `name` to key a conflict off of)
-- ================================================================

INSERT INTO sandbox.customers (name, phone)
SELECT 'Walk-in Customer', '-'
WHERE NOT EXISTS (SELECT 1 FROM sandbox.customers WHERE name = 'Walk-in Customer');
...
-- ================================================================
-- app_meta marker table — identical shape in BOTH schemas so the app
-- can assert which schema it is actually talking to at runtime.
-- ================================================================
...
-- ================================================================
-- sandbox.reset_all() — the only data-clearing primitive in the whole
-- Sandbox feature. Schema-locked via the `schemaname = 'sandbox'`
-- filter below: it is structurally impossible for this to reach any
-- `public` table, no matter what tables sandbox grows in the future.
-- ================================================================
