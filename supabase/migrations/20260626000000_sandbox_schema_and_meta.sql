...
-- `sandbox-setup.sql`, run by hand) into version control. Also adds
-- an `app_meta` marker table to BOTH `public` and `sandbox` so the
-- app can tell which schema it is talking to, and a schema-locked
-- `sandbox.reset_all()` function — the only data-clearing primitive
-- in the sandbox feature, structurally incapable of touching `public`.
--
-- GOING FORWARD CONVENTION: any future migration that changes a
-- `public` table covered by the sandbox mirror must apply the
-- identical DDL to the matching `sandbox` table in the SAME file,
...
-- ================================================================
-- RE-GRANTS (belt-and-braces on top of ALTER DEFAULT PRIVILEGES,
-- so existing objects created above are covered too)
-- ================================================================
...
-- ================================================================
-- APP_META MARKER (both schemas — lets the app confirm which
-- schema it is actually talking to)
-- ================================================================

CREATE TABLE IF NOT EXISTS public.app_meta (
  id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  schema_marker TEXT NOT NULL UNIQUE,
...
VALUES ('00000000-0000-0000-0000-000000000001', 'public', '0.1.61')
...
VALUES ('00000000-0000-0000-0000-000000000002', 'sandbox', '0.1.61')
ON CONFLICT (id) DO UPDATE
  SET app_version = EXCLUDED.app_version,
      updated_at  = now();

-- ================================================================
-- SANDBOX RESET FUNCTION — the only data-clearing primitive in the
-- whole feature. Schema-locked: the loop only ever iterates tables
-- where pg_tables.schemaname = 'sandbox', so it is structurally
-- impossible for it to touch `public`.
-- ================================================================
