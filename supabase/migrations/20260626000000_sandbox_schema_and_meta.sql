...
--
-- SAFE: Does NOT touch any existing `public` data columns.
-- Creates a fully isolated `sandbox` schema mirroring `public`.
-- ================================================================
...
-- ────────────────────────────────────────────────────────────────
-- 1. LOCATIONS  (referenced by many tables — create first)
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 6. SALESPEOPLE  (must be before invoices to allow FK)
-- ────────────────────────────────────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_sandbox_invoices_salesperson_id ON sandbox.invoices(salesperson_id);
...
-- ────────────────────────────────────────────────────────────────
-- 14. LOYALTY TRANSACTIONS  (FK to sales_returns added below)
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 17. RETURNS (legacy customer returns, superseded by sales_returns)
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 27. PURCHASE DISCOUNT APPROVALS
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 31. AUDIT METRICS HOURLY (mirrors public.audit_metrics_hourly)
-- ────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS sandbox.audit_metrics_hourly (
...
ALTER TABLE sandbox.audit_metrics_hourly ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "All access audit_metrics_hourly" ON sandbox.audit_metrics_hourly;
CREATE POLICY "All access audit_metrics_hourly" ON sandbox.audit_metrics_hourly FOR ALL USING (true) WITH CHECK (true);

-- ════════════════════════════════════════════════════════════════
-- EXPLICIT TABLE GRANTS (belt-and-suspenders alongside ALTER DEFAULT PRIVILEGES)
-- ════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════
-- FUNCTIONS  (all scoped to sandbox, SET search_path = sandbox)
-- ════════════════════════════════════════════════════════════════
...
BEGIN
  IF OLD.status IS DISTINCT FROM 'received' AND NEW.status = 'received' THEN
    FOR v_recv IN
      SELECT product_id, received_units, damaged_units, location_id
    ...
    LOOP
      ...
      IF v_sellable > 0 THEN
        ...
        VALUES (v_recv.product_id, v_cartons, v_loose, 'Received from PO: ' || NEW.reference, now(), v_recv.location_id);
      END IF;
    END LOOP;
  END IF;
  RETURN NEW;
...
-- Helper functions from sandbox-patch.sql
...
-- ════════════════════════════════════════════════════════════════
-- APP META — schema identity marker in BOTH schemas
-- ════════════════════════════════════════════════════════════════
...
CREATE TABLE IF NOT EXISTS sandbox.app_meta (
  ...
  app_version    TEXT NOT NULL DEFAULT '0.1.54',
  updated_at     TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT sandbox_app_meta_single_row CHECK (id = 1)
);
...
-- ════════════════════════════════════════════════════════════════
-- sandbox.reset_all() — the ONLY data-clearing primitive for tests
-- Schema-locked: structurally impossible to touch `public` tables
-- ════════════════════════════════════════════════════════════════
