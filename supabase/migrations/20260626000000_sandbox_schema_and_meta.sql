-- ================================================================
-- Diastar Wholesale — Sandbox schema, app_meta marker, guarded reset
-- Migration: 20260626000000
--
-- Going forward convention: any future migration that changes `public`
-- must apply the identical DDL to `sandbox` in the SAME file, so the
-- two schemas never drift again.
--
-- Source of truth for this port: root `sandbox-setup.sql` (the existing
-- hand-run sandbox script), cross-checked column-by-column against the
-- ACTUAL current `public` schema as defined by every file in
-- supabase/migrations/ plus the ad-hoc root scripts that were run by
-- hand directly against production before migration tracking existed
-- (supabase_migration.sql, supplier_module_upgrade.sql,
-- pos_payment_upgrade.sql, supabase_fifo_migration.sql,
-- pos_location_stock_split.sql). Five drifts were found and corrected
-- here (see "PORT FIX" comments below): products.cost_price NOT NULL,
-- customers loyalty + cheque_float columns, invoices.idempotency_key +
-- salesperson_name, payments.cheque_status, stock_batches.original_units.
-- ================================================================
...
CREATE TABLE IF NOT EXISTS sandbox.products (
  ...
  cost_price        NUMERIC(12,2) NOT NULL DEFAULT 0, -- PORT FIX: NOT NULL per 20260515120131_product_cost_price.sql
...
CREATE TABLE IF NOT EXISTS sandbox.customers (
  ...
  is_active               BOOLEAN DEFAULT TRUE,
  loyalty_points          INT NOT NULL DEFAULT 0,             -- PORT FIX: 20260515000000_loyalty_returns_v2.sql
  total_loyalty_earned    INT NOT NULL DEFAULT 0,             -- PORT FIX
  total_loyalty_redeemed  INT NOT NULL DEFAULT 0,             -- PORT FIX
  cheque_float            NUMERIC(12,2) NOT NULL DEFAULT 0,   -- PORT FIX: 20260625000000_cheque_management.sql
  created_at              TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at              TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS sandbox.stock_batches (
  ...
  created_at     TIMESTAMPTZ NOT NULL DEFAULT now(),
  original_units INT                                          -- PORT FIX: 20260623000000_stock_original_units_and_return_salesperson.sql
);
...
CREATE TABLE IF NOT EXISTS sandbox.invoices (
  ...
  notes            TEXT NOT NULL DEFAULT '',
  idempotency_key  TEXT,                                       -- PORT FIX: 20260515000000_loyalty_returns_v2.sql
  salesperson_name TEXT,                                        -- PORT FIX: 20260517000000_salesperson.sql
  created_at       TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at       TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS sandbox.payments (
  ...
  bank_name      TEXT,
  due_date       DATE,
  cheque_status  TEXT CHECK (cheque_status IN ('pending','processing','completed','returned')), -- PORT FIX: 20260625000000_cheque_management.sql
...
-- ================================================================
-- GRANTS (re-asserted; mirrors sandbox-setup.sql's default-privilege grant)
-- ================================================================
...
-- PORT FIX: stamps original_units at insert time, mirrors
-- public.set_stock_batch_original_units() from 20260623000000_stock_original_units_and_return_salesperson.sql
...
-- ================================================================
-- APP_META MARKER (both schemas, never drift independently)
-- ================================================================
...
-- Schema-locked: the loop only ever sees pg_tables rows where
-- schemaname = 'sandbox', so it is structurally impossible for this
-- function to touch `public`, no matter what tables sandbox grows to.
-- ================================================================
