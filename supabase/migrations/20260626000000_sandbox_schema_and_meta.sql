...
--
-- This migration:
--   1. Creates the `sandbox` schema + grants
--   2. Creates all sandbox tables (mirror of public, current state)
--   3. Creates sandbox functions, triggers, and views
--   4. Adds app_meta to BOTH schemas (schema marker + version)
--   5. Adds sandbox.reset_all() — schema-locked, service_role only
-- ================================================================

-- ────────────────────────────────────────────────────────────────
-- 0. Schema + access grants
-- ────────────────────────────────────────────────────────────────
create schema if not exists sandbox;
grant usage on schema sandbox to anon, authenticated, service_role;
alter default privileges in schema sandbox grant all on tables    to anon, authenticated, service_role;
alter default privileges in schema sandbox grant all on sequences to anon, authenticated, service_role;
alter default privileges in schema sandbox grant all on functions to anon, authenticated, service_role;
...
-- ────────────────────────────────────────────────────────────────
-- 10. SALESPEOPLE (before invoices for FK)
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 14. RETURNS (legacy customer returns)
-- ────────────────────────────────────────────────────────────────
...
-- Back-fill FK from loyalty_transactions to sales_returns
do $$
begin
  if not exists (
...
-- ════════════════════════════════════════════════════════════════
-- FUNCTIONS (all scoped to sandbox schema)
-- ════════════════════════════════════════════════════════════════
...
-- Stamp original_units at INSERT time (mirrors public behaviour)
...
-- Restore stock pieces (used by returns)
...
returns jsonb as $$
declare
  v_return record; v_item record; v_ppc int; v_units int;
begin
...
returns jsonb as $$
declare
  v_return   record; v_snapshot jsonb; v_item jsonb; v_ppc int; v_units int;
begin
  select * into v_return from sandbox.sales_returns where id = p_return_id for update;
  if not found then
...
-- ────────────────────────────────────────────────────────────────
-- Explicit table grants (belt-and-suspenders over default privileges)
-- ────────────────────────────────────────────────────────────────
...
-- ════════════════════════════════════════════════════════════════
-- RESET FUNCTION — schema-locked to sandbox, service_role only
-- ════════════════════════════════════════════════════════════════
