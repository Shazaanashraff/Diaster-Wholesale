-- ================================================================
-- DIASTAR ERP — 20260626000000 — Sandbox Schema: First-Class Migration
--
-- Convention: Future migrations that change `public` must apply the
-- identical DDL to `sandbox` in the same file.
--
-- This migration:
--   1. Creates the `sandbox` schema and grants access
--   2. Ports all sandbox tables idempotently (sources: sandbox-setup.sql
--      + sandbox-patch.sql + public migrations 20260515–20260625)
--   3. Adds app_meta marker table to BOTH schemas
--   4. Adds the schema-locked sandbox.reset_all() function
-- ================================================================
...
-- ────────────────────────────────────────────────────────────────
-- 17a. SALESPEOPLE — created early so invoices.salesperson_id can reference it
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 13. RETURNS (legacy customer returns)
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 27. SALES RETURNS (from sandbox-patch.sql, mirrors 20260515000000)
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 28. SALES RETURN ITEMS (from sandbox-patch.sql)
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- 29. LOYALTY TRANSACTIONS (from sandbox-patch.sql, mirrors 20260515000000)
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- ALTER TABLE patches — columns added after initial setup
-- ────────────────────────────────────────────────────────────────

-- customers: loyalty columns (sandbox-patch.sql / 20260515000000)
...
-- customers: cheque_float (20260625000000)
alter table sandbox.customers
  add column if not exists cheque_float numeric(12,2) not null default 0;

-- invoices: idempotency_key (sandbox-patch.sql)
...
-- invoices: salesperson_name (sandbox-patch.sql / 20260517000000)
alter table sandbox.invoices
  add column if not exists salesperson_name text;

-- invoices: salesperson_id (mirrors 20260524000000)
alter table sandbox.invoices
...
-- products: cost_price NOT NULL enforcement (sandbox-patch.sql / 20260515120131)
...
-- payments: cheque_status (20260625000000)
...
-- stock_batches: original_units (mirrors 20260623000000)
alter table sandbox.stock_batches
  add column if not exists original_units int;

-- sales_returns: salesperson_id (mirrors 20260623000000)
alter table sandbox.sales_returns
  add column if not exists salesperson_id uuid references sandbox.salespeople(id) on delete set null;

-- ────────────────────────────────────────────────────────────────
-- FUNCTIONS (from sandbox-setup.sql)
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- FUNCTIONS (from sandbox-patch.sql)
-- ────────────────────────────────────────────────────────────────
...
-- ────────────────────────────────────────────────────────────────
-- TRIGGER: original_units on stock_batches (mirrors 20260623000000)
-- ────────────────────────────────────────────────────────────────
...
-- Backfill original_units for any existing sandbox stock batches
update sandbox.stock_batches sb
set original_units = sb.cartons * greatest(coalesce(p.pieces_per_carton, 1), 1) + sb.loose_pieces
from sandbox.products p
where p.id = sb.product_id
  and sb.original_units is null;

-- ────────────────────────────────────────────────────────────────
-- VIEWS (from sandbox-setup.sql)
-- ────────────────────────────────────────────────────────────────
...
-- ════════════════════════════════════════════════════════════════
-- app_meta — schema-marker table in BOTH schemas
-- Single-row table; idempotent via on conflict (id) do update.
-- ════════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════════
-- sandbox.reset_all() — schema-locked data wipe
-- Truncates every sandbox table except app_meta.
-- The loop guard (schemaname = 'sandbox') makes it structurally
-- impossible for this function to touch any public table.
-- ════════════════════════════════════════════════════════════════
