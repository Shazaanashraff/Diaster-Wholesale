...
--
-- Brings the `sandbox` schema (previously the ad-hoc root file
-- `sandbox-setup.sql`, run by hand and untracked) into migrations, so it
-- is reproducible, version-controlled, and safe to re-run.
--
-- GOING-FORWARD CONVENTION: future migrations that change `public` must
-- apply the identical DDL to `sandbox` in the same file, so the two
-- schemas never drift.
--
-- LOCKED DECISIONS:
--   * Money stays NUMERIC(12,2). No bigint, no ledger.
--   * No runtime schema toggle — VITE_SUPABASE_SCHEMA / npm run dev:sandbox
--     stays the only switch.
--   * This migration only ADDS: the `sandbox` schema, an `app_meta`
--     marker table in both schemas, and a schema-locked reset function.
--     It does not alter any `public` table.
-- ================================================================
...
-- ────────────────────────────────────────────────────────────────
-- 26b. Re-grant privileges explicitly (belt-and-suspenders alongside the
--      ALTER DEFAULT PRIVILEGES above, and idempotent to re-run)
-- ────────────────────────────────────────────────────────────────
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA sandbox TO authenticated, service_role;
GRANT USAGE ON ALL SEQUENCES IN SCHEMA sandbox TO authenticated, service_role;
...
-- ================================================================
-- app_meta MARKER TABLE (both schemas, so a query never accidentally
-- lands in the wrong one without failing loudly)
-- ================================================================
...
CREATE TABLE IF NOT EXISTS sandbox.app_meta (
  ...
  app_version    TEXT NOT NULL,
  updated_at     TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT sandbox_app_meta_singleton CHECK (id = 1)
);
ALTER TABLE sandbox.app_meta DISABLE ROW LEVEL SECURITY;
...
-- ================================================================
-- GUARDED RESET — the only data-clearing primitive in the Sandbox
-- feature. Structurally locked to schemaname = 'sandbox' so it is
-- impossible for it to touch `public`, even if called by mistake.
-- ================================================================
...
BEGIN
  ...
  END LOOP;
END $$;
-- ALTER DEFAULT PRIVILEGES above auto-grants EXECUTE on newly created
-- functions to anon/authenticated too; explicitly strip it back off here
-- so reset_all() is reachable by service_role only.
