...
import { Plus, Search, X, Loader2, AlertCircle, CheckCircle2, Trash2, Edit2, Receipt, TrendingDown, TrendingUp, RefreshCw } from 'lucide-react';
...
export const ExpensesPage: React.FC = () => {
  ...
  const visibleExpenses = useMemo(() =>
    expenses.filter(e => !search || e.category.toLowerCase().includes(search.toLowerCase()) || e.notes.toLowerCase().includes(search.toLowerCase())),
    [expenses, search]);
    ...
    <div className="pos-standard-page p-6 space-y-6 relative">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">Track company expenses and other income</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setFormError(''); tab === 'expenses' ? openAddExpense() : (setIncomeForm({ ...EMPTY_INCOME }), setModalOpen(true)); }} className="flex items-center gap-1.5 px-4 py-2 bg-primary text-white rounded-xl text-xs font-bold hover:bg-primary/90 transition-colors">
            <Plus size={13} /> {tab === 'expenses' ? 'Add Expense' : 'Add Income'}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          { label: 'Net', value: fmt(totalIncome - totalExpenses), accent: totalIncome >= totalExpenses ? 'text-emerald-400' : 'text-red-400', icon: Receipt },
        ].map(k => (
          <div key={k.label} className="bg-[#1d222a] border border-[#2b313a] rounded-2xl p-4 flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl bg-[#2b313a] flex items-center justify-center"><k.icon size={16} className={k.accent} /></div>
            <div>
      ...
      <div className="flex items-center gap-1 bg-[#1d222a] border border-[#2b313a] rounded-xl p-1">
        {(['expenses', 'income'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} className={cn('flex-1 py-2 text-[10px] font-bold uppercase tracking-widest rounded-lg transition-all', tab === t ? 'bg-[#f8fafc] text-black' : 'text-gray-500 hover:text-gray-300')}>
      ...
      <div className="relative">
        ...
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search…" className="w-full bg-[#1d222a] border border-[#2b313a] text-gray-300 text-xs rounded-xl py-2.5 pl-9 pr-4 focus:outline-none focus:border-primary/40" />
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[#2b313a]">
              {visibleExpenses.length === 0 ? (
                ...
                <tr key={e.id} className="hover:bg-[#1d222a] transition-colors" style={{ animation: 'posFadeIn 200ms ease both', animationDelay: `${i * 20}ms` }}>
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => openEditExpense(e)} className="p-1.5 rounded-lg text-gray-500 hover:text-white hover:bg-[#2b313a] transition-colors"><Edit2 size={11} /></button>
                      <button onClick={() => setDeleteTarget({ id: e.id, type: 'expense' })} className="p-1.5 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors"><Trash2 size={11} /></button>
                    </div>
                  </td>
                </tr>
              ))}
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-[#2b313a]">
              {visibleIncome.length === 0 ? (
                ...
                <tr key={inc.id} className="hover:bg-[#1d222a] transition-colors" style={{ animation: 'posFadeIn 200ms ease both', animationDelay: `${i * 20}ms` }}>
                  ...
                  <td className="px-5 py-3.5 text-right">
                    <button onClick={() => setDeleteTarget({ id: inc.id, type: 'income' })} className="p-1.5 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-500/10 transition-colors"><Trash2 size={11} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
          ...
          <div className="fixed inset-0 z-[130] flex items-center justify-center p-4">
            <div className="bg-[#171c23] border border-[#2b313a] rounded-2xl w-full max-w-md shadow-2xl" style={{ animation: 'posFadeIn 180ms ease' }}>
              <div className="flex items-center justify-between p-5 border-b border-[#2b313a]">
                <h3 className="font-bold text-white">{tab === 'expenses' ? (editTarget ? 'Edit Expense' : 'New Expense') : 'Record Other Income'}</h3>
                <button onClick={() => setModalOpen(false)} className="p-1.5 rounded-lg text-gray-500 hover:text-white hover:bg-[#2b313a] transition-colors"><X size={14} /></button>
              </div>
                  ...
                  <>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <select value={form.category} onChange={e => setForm(p => ({ ...p, category: e.target.value }))} className="w-full bg-[#1d222a] border border-[#2b313a] text-gray-300 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:border-primary/40">
                      ...
                      <div>
                        ...
                        <select value={form.method} onChange={e => setForm(p => ({ ...p, method: e.target.value }))} className="w-full bg-[#1d222a] border border-[#2b313a] text-gray-300 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:border-primary/40">
                    ...
                    <div>
                      ...
                      <input type="number" min="0" step="0.01" value={form.amount} onChange={e => setForm(p => ({ ...p, amount: e.target.value }))} placeholder="0.00" className="w-full bg-[#1d222a] border border-[#2b313a] text-gray-300 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:border-primary/40 font-mono" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1">Location</label>
                      <select value={form.location_id} onChange={e => setForm(p => ({ ...p, location_id: e.target.value }))} className="w-full bg-[#1d222a] border border-[#2b313a] text-gray-300 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:border-primary/40">
                    ...
                    <div>
                      ...
                      <input value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} placeholder="What is this expense for?" className="w-full bg-[#1d222a] border border-[#2b313a] text-gray-300 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:border-primary/40" />
                    </div>
                  </>
                ) : (
                  <>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <select value={incomeForm.source_type} onChange={e => setIncomeForm(p => ({ ...p, source_type: e.target.value as any }))} className="w-full bg-[#1d222a] border border-[#2b313a] text-gray-300 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:border-primary/40">
                          {INCOME_SOURCES.map(s => <option key={s.val} value={s.val}>{s.label}</option>)}
                        </select>
                      </div>
                      <div>
                        <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1">Method *</label>
                        <select value={incomeForm.method} onChange={e => setIncomeForm(p => ({ ...p, method: e.target.value }))} className="w-full bg-[#1d222a] border border-[#2b313a] text-gray-300 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:border-primary/40">
                          {PAYMENT_METHODS.map(m => <option key={m} value={m}>{m.replace('_', ' ')}</option>)}
                        </select>
                      </div>
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1">Amount (LKR) *</label>
                      <input type="number" min="0" step="0.01" value={incomeForm.amount} onChange={e => setIncomeForm(p => ({ ...p, amount: e.target.value }))} placeholder="0.00" className="w-full bg-[#1d222a] border border-[#2b313a] text-gray-300 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:border-primary/40 font-mono" />
                    </div>
                    <div>
                      <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1">Supplier (Optional)</label>
                      <select value={incomeForm.supplier_id} onChange={e => setIncomeForm(p => ({ ...p, supplier_id: e.target.value }))} className="w-full bg-[#1d222a] border border-[#2b313a] text-gray-300 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:border-primary/40">
                    ...
                    <div>
                      <label className="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-1">Notes *</label>
                      <input value={incomeForm.notes} onChange={e => setIncomeForm(p => ({ ...p, notes: e.target.value }))} placeholder="Description of income…" className="w-full bg-[#1d222a] border border-[#2b313a] text-gray-300 text-xs rounded-xl px-3 py-2.5 focus:outline-none focus:border-primary/40" />
                    </div>
                  </>
                )}
              </div>
              <div className="flex gap-3 p-5 border-t border-[#2b313a]">
                <button onClick={() => setModalOpen(false)} className="flex-1 py-2.5 bg-[#1d222a] border border-[#2b313a] text-gray-400 rounded-xl text-xs font-bold hover:text-white transition-colors">Cancel</button>
                <button onClick={tab === 'expenses' ? handleSaveExpense : handleSaveIncome} disabled={saving} className="flex-1 py-2.5 bg-primary text-white rounded-xl text-xs font-bold hover:bg-primary/90 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
